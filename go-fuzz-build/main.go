...
import (
	...
	. "github.com/guidovranken/libfuzzer-go/go-fuzz-defs"
)
...
func main() {
	...
	for _, p := range goListList("github.com/guidovranken/libfuzzer-go/go-fuzz-dep", "Deps") {
		if p == "go-fuzz-defs" {
			continue
		}
		deps[p] = true
...
func buildInstrumentedBinary(outf string, pkg string, deps map[string]bool, lits map[Literal]struct{}, blocks *[]CoverBlock, sonar *[]CoverBlock) string {
	...
	copyDir(filepath.Join(GOROOT, "src", "runtime", "cgo"), filepath.Join(workdir, "goroot", "src", "runtime", "cgo"), true, nil)
	...
	os.Remove(outf)
	cmd := exec.Command("go", "build", "-tags", makeTags(), "-buildmode=c-archive", "-o", outf, mainPkg)
	for _, v := range os.Environ() {
...
func copyFuzzDep(workdir string, std bool) {
	...
	clonePackage(workdir, "github.com/guidovranken/libfuzzer-go/go-fuzz-dep", "go-fuzz-dep", std)
	clonePackage(workdir, "github.com/guidovranken/libfuzzer-go/go-fuzz-defs", "go-fuzz-defs", std)
}
